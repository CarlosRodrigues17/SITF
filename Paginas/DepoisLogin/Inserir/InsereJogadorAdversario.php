...
//Query para obter posiÃ§Ãµes na lista
mysql_select_db($database_localhost, $localhost);
$query_Posicao = "SELECT * FROM posicao";
$Posicao = mysql_query($query_Posicao, $localhost) or die(mysql_error());
$row_Posicao = mysql_fetch_assoc($Posicao);
$totalRows_Posicao = mysql_num_rows($Posicao);

//Query para obter Ãºltimo id de jogador na Base de dados
mysql_select_db($database_localhost, $localhost);
...
//Query obter lista os clubes AdversÃ¡rios
mysql_select_db($database_localhost, $localhost);
$query_ListaClubes = "SELECT * FROM clube WHERE Id_Clube <> $identificacao";
$ListaClubes = mysql_query($query_ListaClubes, $localhost) or die(mysql_error());
$row_ListaClubes = mysql_fetch_assoc($ListaClubes);
$totalRows_ListaClubes = mysql_num_rows($ListaClubes);
...
//VariÃ¡vel para guardar o Ãºltimo id de jogador
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "FormJogador")) {
  $insertSQL = sprintf("INSERT INTO jogador (Id_Jogador, Id_Clube,Nome_Completo, Nome_Conhecido, Data_de_Nascimento,Idade, Naturalidade, Nacionalidade, Peso, Altura, Pe_Preferido, Numero_da_camisola, Contracto, Foto) VALUES ($Ult_id_jogador,%s,%s, %s, %s,$idade, %s, %s, %s, %s, %s, %s, %s, %s)",
  					   GetSQLValueString($_POST['Equipa'], "int"),
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "FormJogador")) {
  $insertSQL = sprintf("INSERT INTO jogador_posicao (Id_jogador,Sigla) VALUES ($Ult_id_jogador,%s)",
                       GetSQLValueString($_POST['Posicao1'], "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "FormJogador")) {
  $insertSQL = sprintf("INSERT INTO jogador_posicao (Id_jogador,Sigla) VALUES ($Ult_id_jogador,%s)",
                       GetSQLValueString($_POST['Posicao2'], "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "FormJogador")) {
  $insertSQL = sprintf("INSERT INTO jogador_posicao (Id_jogador,Sigla) VALUES ($Ult_id_jogador,%s)",
                       GetSQLValueString($_POST['Posicao3'], "text"));
...
<div id="Conteudo"> 
  ...
  <label id="LabelNomeForm">Inserir Jogador AdversÃ¡rio</label>
  
  <form action="<?php echo $editFormAction; ?>" method="POST" name="FormJogador">
  
  <label id="LabelEquipa">Equipa:</label>
  <Select name="Equipa" id="Equipa"/>
    <option value="">Escolher</option>
        <?php
do {  
?>
        <option value="<?php echo $row_ListaClubes['Id_Clube']?>"><?php echo $row_ListaClubes['Nome']?></option>
        <?php
} while ($row_ListaClubes = mysql_fetch_assoc($ListaClubes));
  $rows = mysql_num_rows($ListaClubes);
  if($rows > 0) {
      mysql_data_seek($ListaClubes, 0);
	  $row_ListaClubes = mysql_fetch_assoc($ListaClubes);
  }
?>  
   </Select>
...
?>  
  ...
  <Select name="Posicao1" id="Posicao1"/>
    <option value="">Escolher</option>
        <?php
do {  
?>
        <option value="<?php echo $row_Posicao['Sigla']?>"><?php echo $row_Posicao['Nome']?></option>
        <?php
} while ($row_Posicao = mysql_fetch_assoc($Posicao));
  $rows = mysql_num_rows($Posicao);
  if($rows > 0) {
      mysql_data_seek($Posicao, 0);
	  $row_Posicao = mysql_fetch_assoc($Posicao);
  }
?>  
   </Select>
   
  <Select name="Posicao2" id="Posicao2"/>
    <option value="">Escolher</option>
        <?php
do {  
?>
...
?>  
  ...
  <Select name="Posicao3" id="Posicao3"/>
    <option value="">Escolher</option>
        <?php
do {  
?>
