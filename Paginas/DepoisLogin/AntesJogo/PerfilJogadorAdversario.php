...
//Guardar uma variável com o id do Adversário se necessário
$IdAdversario;
if(empty($_GET['Id'])){
...
mysql_select_db($database_localhost, $localhost);
$query_ListaJogadores = "SELECT Nome_Conhecido FROM jogador WHERE Id_Clube = $IdAdversario ORDER BY jogador.Numero_da_camisola";
...
if(empty($_POST["SelecJogador"])&&empty($_GET['nome'])){
	$JogadorNome = "";
	}else if(empty($_POST["SelecJogador"])&&$_GET['nome']){
		$JogadorNome = mysql_escape_string(strtr($_GET['nome'],"-"," ")); 					
		}else{
			$JogadorNome = mysql_escape_string($_POST['SelecJogador']);
		}
//Query para obter as informações do Jogador selecionado 
mysql_select_db($database_localhost, $localhost);
$query_Jogador = "SELECT * FROM jogador WHERE Nome_Conhecido LIKE '%$JogadorNome%'  AND jogador.Id_Clube = $IdAdversario";
...
$id_Jogador = $row_Jogador['Id_Jogador'];

//Query para a posição dos jogadores
mysql_select_db($database_localhost, $localhost);
$query_PosicaoJogador = "SELECT Sigla FROM jogador_posicao WHERE Id_Jogador =$id_Jogador ";
...
do {  
?>
        <option value="<?php echo $row_ListaJogadores['Nome_Conhecido']?>"><?php echo $row_ListaJogadores['Nome_Conhecido']?></option>
        <?php
